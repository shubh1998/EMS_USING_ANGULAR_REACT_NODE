...
import { deleteEmployeeRequest, fetchEmployeesListRequest } from '../../../utils/services/employee.service';
...
export const EmployeeList = observer(() => {
    ...
    const {employees} = useAppStoreContext()
    ...
    const fetchAllEmployees = async(value) => {
      const result = await fetchEmployeesListRequest(value)
      if(result){
        employees.setEmployeeList(result);
      }
    }
    ...
    useEffect(() => {
      if(!employees.employeeList.length){
        fetchAllEmployees(filter)
      }
    }, [filter]);

    useEffect(()=>{
      setTableData(employees.employeeList);
